...
//   // 1) Local state for products
//   const [products, setProducts] = useState([]);

//   // 2) State for new product form
...
//   // 3) Load products from localStorage on component mount
//   useEffect(() => {
//     const storedProducts = JSON.parse(localStorage.getItem('products')) || [];
//     setProducts(storedProducts);
//   }, []);

//   // 4) Whenever "products" changes, save to localStorage
//   useEffect(() => {
...
//       // Provide a default image or extra fields as needed
//       imageUrl: 'https://via.placeholder.com/400x300.png?text=New+Product',
//       // You can add more fields here (description, rating, etc.) if you like
//     };
...
function AdminDashboard() {
  ...
  // 2) Local states for products + form input
  const [products, setProducts] = useState([]);
  ...
  // Check if admin is logged in (just an example)
  useEffect(() => {
    const token = localStorage.getItem('token');
    if (!token) {
      navigate('/admin/login');
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        // get all docs from that collection
        const snapshot = await getDocs(productsCollection);

        const loadedProducts = [];
        snapshot.forEach((docSnapshot) => {
          // docSnapshot.data() is the data, docSnapshot.id is the doc ID
          loadedProducts.push({ id: docSnapshot.id, ...docSnapshot.data() });
        });
  ...
  // 4) CREATE a new product in Firestore
  const handleCreate = async (e) => {
    ...
    try {
      ...
      // docRef.id is the new doc's ID from Firestore
      ...
      // Update state so it shows up immediately
  ...
  // 5) DELETE product from Firestore
